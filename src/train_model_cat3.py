...
if __name__ == '__main__':  
    # 1) Load dataset
    f = open("./../datasets/CPMG_G_XY_7_nl.ds", 'rb')
    data = pickle.load(f)
    f.close()  
    ...
    mlmodel = qubitMLmodel(T/M, Omega, "Multi_Axis",2)
    
    # 4) Perform training 
    mlmodel.train_model_val(training_inputs, training_targets, testing_inputs, testing_targets, 3000)    
    
    # 5) Save results        
    mlmodel.save_model("trained_model_CPMG_G_XY_7_nl_3000")
#########################################################
    
    # 1) Load dataset
    f = open("./../datasets/CPMG_G_XY_pi_7_nl.ds", 'rb')
    data = pickle.load(f)
    f.close()  
    ...
    # 3)  Define the ML model
    mlmodel = qubitMLmodel(T/M, Omega, "Multi_Axis",1)
    
    # 4) Perform training 
    mlmodel.train_model_val(training_inputs, training_targets, testing_inputs, testing_targets, 3000)    
    
    # 5) Save results        
    mlmodel.save_model("trained_model_CPMG_G_XY_pi_7_nl_3000")
